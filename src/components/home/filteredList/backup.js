...
const FilteredList = (filter) => {
  ...
  return (
    <div className="shoesList">
      {listShoes.map((item) => {
        const { id, name, fullPrice, price, image, sizes, brand, description} = item;
        console.log(filter)
        console.log(brand)
        if (brand === filter.filter || filter.filter === 'all'){
        return (
          <div className="item" key={`list ${id}`}>               
            <div className="image">
              <img src={image} alt={name} onClick = {() => setisModalVisible(true)}/>
